	...
	_title()
{
	...
	_volumeSlider.setPosition({ _renderWindow.getSize().x / 2.f - _volumeSlider.getSize().x / 2.f, 360.f });
	...
	_backButton.setActionTodo([&]() { _stateManager.popState(); });
	...
	_title.setPosition(_renderWindow.getSize().x / 2.f, 100.f);
}
